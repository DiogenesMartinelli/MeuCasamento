...
const schema = z.object({
  ...
  confirmedMessage: z.string().trim().max(500).optional().or(z.literal("")).transform((v) => (v ? v : undefined)),
...
export async function updateRsvpTheme(
  ...
  const parsed = schema.safeParse({
    ...
    confirmedMessage: formData.get("confirmedMessage") ?? "",
  ...
  try {
    ...
    const data = {
      ...
      confirmedMessage: parsed.data.confirmedMessage ?? null,
