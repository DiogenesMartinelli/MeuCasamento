...
/** One-time fee to unlock account creation. Platform revenue, paid before any Account exists. */
...
export async function startRegistrationCheckout(
  ...
  const registrationPayment = await prisma.registrationPayment.create({
    data: { email, amount: REGISTRATION_FEE },
  });
  ...
  try {
    ...
    const result = await preference.create({
      body: {
        ...
        payer: { email },
        payment_methods: {
          // Card-only: this is the platform's own signup fee, not a Pix/boleto gift.
          excluded_payment_types: [{ id: "ticket" }, { id: "bank_transfer" }, { id: "atm" }],
        },
        back_urls: {
          success: `${appUrl}/comecar/retorno?ref=${registrationPayment.id}`,
          pending: `${appUrl}/comecar/retorno?ref=${registrationPayment.id}`,
          failure: `${appUrl}/comecar/retorno?ref=${registrationPayment.id}`,
        },
        auto_return: autoReturnFor(appUrl),
        notification_url: `${appUrl}/api/webhooks/mercadopago`,
        external_reference: registrationPayment.id,
      },
    });
